     ...
     <tbody id="customer_table">
                      <?php $i = 1; if(isset($orders_list)){ foreach($orders_list as $row){ ?>
                        ...
                        <?php if($row->order_status == 'delivered'){?> <p class="status" style="background-color:#019727; ">Delivered</p> <?php } ?>     
                           <?php if($row->order_status == 'canceled'){?> <p class="status" style="background-color:#0088cc; ">Canceled</p> <?php } ?>

                         </td>
                            <td><select class="form-control"  name="delivery_person"  style="height:25px; padding:0px 5px;">
                                  <option value="">Delivery Person</option>
                                  <?php if(isset($all_delivery_person)){ foreach($all_delivery_person as $my_row){ ?>
    
                                   <option value="<?php echo $my_row->staff_id; ?>" ><?php echo $my_row->name; ?> </option>
                                   <?php } } ?>
                                </select>
                            </td>  
...
<script type="text/javascript">
     ...
     // set scroll position
        ...
        $(document).on('change','select[name=change_order_status]',function(){
            ...
            var delivery_person = $(this).parent().parent().find('select[name=delivery_person]').val();
            
            var href = window.location.href;
            var return_href = href.substring(href.lastIndexOf('/'));
            
            if(delivery_person === ''){
                $('select[name=delivery_person]').css('border','1px solid #424351');
                $(this).parent().parent().find('select[name=delivery_person]').css('border','1px solid red');
            }
            
            if(status && order_id && delivery_person){
                window.location.href = '<?php echo base_url(); ?>orders/order_dispatch_and_assign/'+order_id+'/'+status+'/'+delivery_person+'/'+return_href;
            }
        ...
        $(document).on('change','select[name=change_order_status]',function(){
            ...
            //alert(return_href);
            if(delivery_person === '' && status == 'dispatched'){
                $('select[name=delivery_person]').css('border','1px solid #424351');
                $(this).parent().parent().find('select[name=delivery_person]').css('border','1px solid red');
            ...
            if(status && order_id && delivery_person){
              ...
              $.ajax({
                        ...
                        type: 'POST',
						url: '<?php echo base_url(); ?>orders/order_dispatch_and_assign',
						data: {status:status,order_id:order_id,delivery_person:delivery_person},
						beforeSend: function(){
                            ...
                            if(data === 'success'){
                                ...
                                if(status === 'placed'){    
                                  ...
                                  this_eliment.css('border','1px solid #cfd22d');
                                    ...
                                    //var j = this_eliment.parent().patent().html();
                                    //alert(j);
                                          this_eliment.parent().parent().find('.status_td').html('<p class="status" style="background-color:#f8cd21; width:80px; text-align:center; color:#ffffff; font-weight:700; border-radius:30px;">Placed</p>');
                                     
                                }else if(status === 'canceled'){
                                    this_eliment.css('border','1px solid #fd1e34');
                                     this_eliment.parent().parent().find('.status_td').html('<p class="status" style="background-color:#ff4748; width:80px; text-align:center; color:#ffffff; font-weight:700; border-radius:30px;">Canceled</p>');
                                }else if(status === 'dispatched'){
                                    alert("s");
                                    this_eliment.css('border','1px solid #2eed3d');
                                    
                                     this_eliment.parent().parent().find('.status_td').html('<p class="status" style="background-color:#e335a7; width:80px; text-align:center; color:#ffffff; font-weight:700; border-radius:30px;">Dispatched</p>');
                                }
                            }
                        }
                });  
