						...
						<section class="section_card" id="table_section">
								 <header class="panel-heading" style="padding:10px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
													...
													<a href="<?Php echo base_url(); ?>users/add"><button type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">Add User</button></a>
									    </div>
								</header>
                          ...
                          <tr>
                          	  ...
                          	  <td><?php echo $row->email_address; ?></td>
														  ...
														  <td style=" text-align:center;"><a href="<?php echo base_url(); ?>users/edit/<?php echo $row->staff_id; ?>"><button class="btn btn-primary" id=""  data-edit_id="" style="color:#248afd; "><i class="fa fa-pencil"></i></button></a></td>
															<td style=" text-align:center;"><a href="<?php echo base_url(); ?>users/edit_user_permission/<?php echo $row->staff_id; ?>"><button class="btn btn-primary" id=""  data-edit_id="" style="color:#f4a70a; "><i class="fa fa-key" style="color:#f4a70a; "></i></button></a></td>
										      	  <td style="text-align:center;"><button type="button" class="btn btn-primary" name="confirm_del_action" data-confirm_del_id="<?php echo $row->staff_id; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button></td>
                          </tr>
										<?php } ?>
									</tbody>
								</table>
    ...
    $(document).on('click','button[name=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   ...
			   var del_id = $(this).data("del_id");
							...
							$.ajax({
		              url : '<?php echo base_url(); ?>/users/del_admin',
								  method: 'POST',
									data:{del_id: del_id},
									success:function(data){
