			  ...
			  <?php $this->load->view('common/sidemenu'); ?>
				...
				<section role="main" class="content-body" >
						...
						<section class="section_card" id="table_section">
                          ...
                          <tr>
														  <td><?php echo $i++; ?></td>
														  <td style="width:110px; padding:4px; text-align:center;"><img src="<?php echo base_url(); ?>uploads/app_background/<?php echo $row->img; ?>" style="border-radius:2px; border:1px solid #45496d;" width="100" ></td>
														  <td><?php echo $row->status; ?></td>
														  <td style=" text-align:center;"><button class="btn btn-primary" id="edit_bt"  data-edit_id="<?php echo $row->sr_no; ?>" style="color:#248afd; "><i class="fa fa-pencil"></i></button></td>
										      	  <td style="text-align:center;"><button type="button" class="btn btn-primary" id="confirm_del_action" data-confirm_del_id="<?php echo $row->sr_no; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button></td>
                          </tr>
										<?php } ?>
									</tbody>
								</table>
					...
					<section class="section_card floating_form">
						 ...
						 <div class="panel-body" style="padding-left:0px; padding-right:0px;  background-color:#27293d; padding-top:15px; padding-bottom:0px;">
											 ...
											 <div class="form-group">
												 <div class="col-md-12">
													<select class="form-control" name="status">
														<option value="active" <?php if(isset($selected_row[0])){ if($selected_row[0]->status == 'active'){ echo 'selected'; }} ?> >Active</option>
														<option value="inactive" <?php if(isset($selected_row[0])){ if($selected_row[0]->status == 'inactive'){ echo 'selected'; }} ?> >Inactive</option>
													</select>
												</div>
											</div>
						       </div>
									 <div class="col-md-12">
										  <div class="form-group">
											   <div class="col-md-12">
												 <input type="file" name="file" id="file" accept="image/*" value="" data-img_folder_name="app_background" style="display:none;"/>
												 <label for="file" id="img_label" style="width:150px; height:150px; border:1px solid #424351; text-align:center;  font-size:25px;"><?php if(isset($selected_row[0])){ ?> <img src="<?php echo base_url(); ?>uploads/app_background/<?php echo $selected_row[0]->img; ?>" data-img_name="" alt="" style="width:100% ; height:100%;"/><input type="hidden" name="image_name" value="<?php echo $selected_row[0]->img; ?>" /><button type="button" class="btn" id="temprory_remove_image" style="position:absolute; background-color:#ff4747; color:#ffffff; margin-top:-2px;">X</button>  <?php }else{ ?> <i class="fa fa-globe" style="margin-top:60px; margin-right:5px;"></i><span style="font-size:12px; ">Browse</span> <?php  } ?>  </label>
											   </div>
...
// rediren for update section
    $(document).on('click','button[id=edit_bt]',function(){
              ...
              var redirect_link  = '<?php echo base_url(); ?>app_background/edit/'+edit_id;
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/app_background/del_row',
								  method: 'POST',
									data:{del_id: del_id},
									success:function(data){
										if(data === 'success'){
