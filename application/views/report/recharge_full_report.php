								 ...
								 <header class="panel-heading" style="padding:0px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
														...
														Recharge Full Report <i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i> <?php if(isset($return_date)){ echo date('d-M-Y',strtotime($return_date)); } ?>
													</h2>
									    </div>
								</header>
    ...
    <tr>
     ...
     <tbody id="customer_table">
                      <?php $i = 1; if(isset($list)){ $sum_amount = 0; foreach($list as $row){ ?>
                       ...
                       <a href="<?php echo base_url(); ?>report/transaction_full_report/<?php echo $row->recharge_date; ?>"> <tr>

                         <td><?php echo $i++; ?></td>
                         <td><?php echo date('d-M-Y',strtotime($row->recharge_date)); ?></td>
                         <td><?php echo date('H:i:s a',strtotime($row->recharge_date)); ?></td>

                         <td><?php echo $row->customer_name; ?></td>
                         <td><?php echo $row->contact_1; ?></td>  
                         <td><span class="rs_span"><i class="fa fa-rupee"></i> </span> <?php echo + $row->recharge_amount; $sum_amount += $row->recharge_amount; ?></td>   
