						...
						<section class="section_card" id="table_section">
								 <header class="panel-heading" style="padding:10px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
													...
													<a href="<?Php echo base_url(); ?>branch/add"><button type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">Add Branch</button></a>
									    </div>
								</header>
                          ...
                          <tr>
                          	  ...
                          	  <td><?php echo $row->outlet_address; ?></td>
														  ...
														  <td style=" text-align:center;"><a href="<?php echo base_url(); ?>branch/edit/<?php echo $row->outlet_id; ?>"><button class="btn btn-primary" id=""  data-edit_id="" style="color:#248afd; "><i class="fa fa-pencil"></i></button></a></td>
								  ...
								  method: 'POST',
									...
									success:function(data){
										...
										if(data === 'success'){
                                            ...
                                            $('.fail_model_p').text('Before deleting this Branch, delete all the users and delivery person related to it.');
                                            $('#del_alert_action').modal("toggle");
                                            $('#failed_alert').modal("toggle");
                                        }else if(data === 'product_category_exist'){
                                            
                                            $('.fail_model_p').text('Before deleting this Branch, delete all the product category related to it.');
                                            $('#del_alert_action').modal("toggle");
                                            $('#failed_alert').modal("toggle");
                                        }
