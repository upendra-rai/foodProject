								 ...
								 <header class="panel-heading" style="padding:10px 15px;">
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
													...
													<a href="<?Php echo base_url(); ?>promo_code/add"><button type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">+ Add </button></a>
									    </div>
								</header>
										  ...
										  <?php $i = 1; foreach($list as $row){ ?>
                                                  <tr>
														  ...
														  <td><?php echo $row->promo_detail; ?></td>
															...
															<td><?php echo date('d M Y',strtotime($row->validate_from)); ?></td>
                                                             <td><?php echo date('d M Y',strtotime($row->validate_to)); ?></td>
                                                             
                                                            <td><?php if($row->promo_status == ''){ echo '<p>active</p>'; }else{ echo '<p style="color:red;">'.$row->promo_status.'</p>'; } ?></td>
														  <td style=" text-align:center;"><a href="<?php echo base_url(); ?>promo_code/edit/<?php echo $row->promo_id; ?>"><button class="btn btn-primary" id=""  data-edit_id="" style="color:#248afd; "><i class="fa fa-pencil"></i></button></a></td>
															
										      	  <td style="text-align:center;"><button type="button" class="btn btn-primary" id="confirm_del_action" data-confirm_del_id="<?php echo $row->promo_id; ?>" style="color:#ff4747; "><i class="fa  fa-times-circle"></i></button></td>
                                                 </tr>
										<?php } ?>
									</tbody>
								</table>
