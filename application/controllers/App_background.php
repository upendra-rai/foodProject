...
class app_background extends CI_Controller {
	...
	function __construct(){
		...
		parent::__construct();
         ...
         $this->load->model('model_app_background');
	...
	public function add()
	{
       ...
       $img_name = $this->input->post('image_name');
			 $data["msg"] = $this->model_app_background->add($status,$img_name);

		}else{
		$data['active_menu'] = "management";
		$data['active_submenu'] = "app_background";

		$data["list"] = $this->model_app_background->select_list();
		$this->load->view('banner/app_background', $data);
	}
	}
	...
	public function edit($id)
	{
       ...
       $last_uploaded_image_name =  $this->input->post('last_uploaded_image_name');
			 ...
			 if($last_uploaded_image_name != $img_name){
				  unlink('uploads/app_banner_image/'.$unlink_img_src);
		  	}
				$data["msg"] = $this->model_app_background->edit($id,$status,$img_name);
		}else{
		$data['active_menu'] = "management";
		$data['active_submenu'] = "app_background";

		$data["selected_row"] = $this->model_app_background->selected_row($id);
		$data["list"] = $this->model_app_background->select_list();
		$this->load->view('banner/app_background', $data);
	  }
	}
	...
	public function del_row()
	{
		if(isset($_POST["del_id"])){
			$del_id = $_POST["del_id"];
			$data["msg"] = $this->model_app_background->del_row($del_id);
		}
